...
#define REQUIRE(status)                                                       \
  {                                                                           \
    if (!(status)) {                                                          \
      std::cerr << "file: " << __FILE__ << ", line: " << __LINE__ << " error" \
                << std::endl;                                                 \
      std::abort();                                                           \
    }                                                                         \
  }
...
static void initImage(int8_t* image, int64_t imageSize) {
  ...
  for (int64_t index = 0; index < imageSize; index++) {
    seed = (1103515245 * seed + 12345) & 0xffffffff;
    // Takes floats from [0, 1), scales and casts to ints from [0, 4], then subtracts
    // from 2
...
static void initImage(int32_t* image, int64_t imageSize) {
  ...
  for (int64_t index = 0; index < imageSize; index++) {
    seed = (1103515245 * seed + 12345) & 0xffffffff;
    // Takes floats from [0, 1), scales and casts to ints from [0, 4], then divides
    // by 4
...
struct Surface {
 ...
 public:
  explicit Surface(int64_t n_elems, [[maybe_unused]] bool hasRef)
      : n_elems(n_elems) {
    CUDA_CHECK(
    ...
    CUDA_CHECK(cudaMemcpy(devPtr, hostPtr, size_t(sizeof(hostPtr[0]) * n_elems),
  ...
  explicit Surface(int64_t size, [[maybe_unused]] bool hasRef, T_ELEM fillValue)
      : n_elems(size) {
    ...
    CUDA_CHECK(cudaMemcpy(devPtr, hostPtr, sizeof(hostPtr[0]) * n_elems,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaDeviceSynchronize());
  }
