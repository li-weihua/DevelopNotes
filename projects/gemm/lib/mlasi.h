...
#include <arm_neon.h>
    ...
    !defined(signature_AMD_ebx)  // workaround for Bug 96238 - [i386] cpuid.h header
                                 // needs include guards
#include <cpuid.h>
#endif
#if defined(__GNUC__) && __GNUC__ >= 12
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored \
    "-Wmaybe-uninitialized"  // GCC 12 warns about uninitialized variables in
                             // immintrin.h.
#include <immintrin.h>
...
#else
#define MLAS_DECLSPEC_ALIGN(variable, alignment) \
...
MLAS_FORCEINLINE
void MlasPrintFinalMessage(const std::string& msg) {
#if defined(__ANDROID__)
  __android_log_print(ANDROID_LOG_ERROR, "mlas", "%s", msg.c_str());
#else
  // TODO, consider changing the output of the error message from std::cerr to
  // logging when the exceptions are disabled, since using std::cerr might increase
  // binary size, and std::cerr output might not be easily accesible on some systems
  // such as mobile
  // TODO, see if we need to change the output of the error message from std::cerr to
  // NSLog for iOS
...
enum MlasUArch {
  ...
  /** ARM Cortex-A55 revision 0 (restricted dual-issue capabilities compared to
     revision 1+). */
...
typedef size_t(MLASCALL MLAS_GEMM_FLOAT_KERNEL)(const float* A, const float* B,
                                                float* C, size_t CountK,
                                                size_t CountM, size_t CountN,
                                                size_t lda, size_t ldc, float alpha,
                                                bool ZeroMode);

typedef size_t(MLASCALL MLAS_GEMM_DOUBLE_KERNEL)(const double* A, const double* B,
                                                 double* C, size_t CountK,
                                                 size_t CountM, size_t CountN,
                                                 size_t lda, size_t ldc,
                                                 double alpha, bool ZeroMode);

#else

#if defined(__aarch64__) && defined(__linux__)
typedef size_t(MLASCALL MLAS_SBGEMM_FLOAT_KERNEL)(const float* A,
                                                  const bfloat16_t* B, float* C,
                                                  size_t CountK, size_t CountM,
                                                  size_t CountN, size_t lda,
                                                  size_t ldc, const float* Bias);
#endif
...
typedef void(MLASCALL MLAS_GEMV_FLOAT_KERNEL)(const float* A, const float* B,
                                              float* C, size_t CountK, size_t CountN,
                                              size_t ldb, bool ZeroMode);

typedef void(MLASCALL MLAS_SGEMM_KERNEL_M1_ROUTINE)(const float* A, const float* B,
                                                    float* C, size_t CountK,
                                                    size_t CountN, size_t ldb,
                                                    float beta);

typedef void(MLASCALL MLAS_SGEMM_TRANSPOSE_PACKB_BLOCK_ROUTINE)(float* D,
                                                                const float* B,
                                                                size_t ldb);

typedef size_t(MLASCALL MLAS_GEMM_U8S8_KERNEL)(
    const uint8_t* A, const uint8_t* B, int32_t* C, size_t PackedCountK,
    size_t CountM, size_t CountN, size_t ldc, const int32_t* RowSumVector,
    const int32_t* ColumnSumVector, const int32_t* ZeroPointB, bool ZeroMode);

typedef size_t(MLASCALL MLAS_GEMV_U8S8_KERNEL)(const uint8_t* A, const uint8_t* B,
                                               int32_t* C, size_t CountK,
                                               size_t CountN, size_t ldb);

typedef size_t(MLASCALL MLAS_GEMM_U8U8_KERNEL)(
    const int16_t* A, const uint8_t* B, int32_t* C, size_t PackedCountK,
    size_t CountM, size_t CountN, size_t ldc, const int32_t* RowSumVector,
    const int32_t* ColumnSumVector, const int32_t* ZeroPointB, bool ZeroMode);

typedef void(MLASCALL MLAS_CONV_FLOAT_KERNEL)(
    const float* Input, const float* Filter, float* Output, size_t StrideWidth,
    size_t DilationWidth, size_t FilterCount, size_t InputStride,
    size_t FilterStride, size_t OutputStride, size_t KernelHeight,
    size_t KernelWidth, const float* InputBase, size_t InputWidth,
    size_t DilatedInputWidth, size_t OutputCountLeftPad, size_t OutputCount,
    size_t OutputCountRightPad, const float* Bias, unsigned KernelFlags);

typedef void(MLASCALL MLAS_CONV_DEPTHWISE_FLOAT_KERNEL)(
    const float* Input, const float* Filter, float* Output, size_t StrideWidth,
    size_t DilationWidth, size_t InputStride, size_t KernelHeight,
    size_t KernelWidth, const float* InputBase, size_t InputWidth,
    size_t DilatedInputWidth, size_t OutputCountLeftPad, size_t OutputCount,
    size_t OutputCountRightPad, const float* Bias, unsigned KernelFlags);
...
typedef void(MLASCALL MLAS_CONV_POINTWISE_FLOAT_KERNEL)(
    const float* Input, const float* Filter, float* Output, size_t StrideWidth,
    size_t InputChannels, size_t FilterCount, size_t InputStride,
    size_t FilterStride, size_t OutputStride, size_t OutputCount, const float* Bias,
    unsigned KernelFlags);

typedef void(MLASCALL MLAS_POOL_FLOAT_KERNEL)(
    const float* Input, float* Output, size_t StrideWidth, size_t DilationWidth,
    size_t InputStride, size_t ActualKernelSize, size_t KernelHeight,
    size_t KernelWidth, const float* InputBase, size_t InputWidth,
    size_t DilatedInputWidth, size_t OutputCountLeftPad, size_t OutputCount,
    size_t OutputCountRightPad);

typedef void(MLASCALL MLAS_COMPUTE_UNARY_FLOAT_KERNEL)(const float* Input,
                                                       float* Output, size_t N);

typedef float(MLASCALL MLAS_COMPUTE_SUMEXP_FLOAT_KERNEL)(
    const float* Input, float* Output, size_t N, const float* NegativeMaximum);
...
typedef float(MLASCALL MLAS_REDUCE_MAXIMUM_FLOAT_KERNEL)(const float* Input,
                                                         size_t N);

typedef void(MLASCALL MLAS_REDUCE_MINIMUM_MAXIMUM_FLOAT_KERNEL)(const float* Input,
                                                                float* Min,
                                                                float* Max,
                                                                size_t N);

typedef void(MLASCALL MLAS_QLINEAR_BINARY_OP_S8_KERNEL)(
    const int8_t* InputA, float ScaleA, int32_t ZeroPointA, const int8_t* InputB,
    float ScaleB, int32_t ZeroPointB, float ScaleC, int32_t ZeroPointC,
    int8_t* OutputC, size_t N, bool IsScalarB);

typedef void(MLASCALL MLAS_QLINEAR_BINARY_OP_U8_KERNEL)(
    const uint8_t* InputA, float ScaleA, int32_t ZeroPointA, const uint8_t* InputB,
    float ScaleB, int32_t ZeroPointB, float ScaleC, int32_t ZeroPointC,
    uint8_t* OutputC, size_t N, bool IsScalarB);

typedef void(MLASCALL MLAS_QUANTIZE_LINEAR_U8_KERNEL)(const float* Input,
                                                      uint8_t* Output, size_t N,
                                                      float Scale,
                                                      uint8_t ZeroPoint);

typedef void(MLASCALL MLAS_QUANTIZE_LINEAR_S8_KERNEL)(const float* Input,
                                                      int8_t* Output, size_t N,
                                                      float Scale, int8_t ZeroPoint);

typedef void(MLASCALL MLAS_QUANTIZE_LINEAR_U16_KERNEL)(const float* Input,
                                                       uint16_t* Output, size_t N,
                                                       float Scale,
                                                       uint16_t ZeroPoint);

typedef void(MLASCALL MLAS_QUANTIZE_LINEAR_S16_KERNEL)(const float* Input,
                                                       int16_t* Output, size_t N,
                                                       float Scale,
                                                       int16_t ZeroPoint);

typedef void(MLASCALL MLAS_QUANTIZE_LINEAR_U4_KERNEL)(const float* Input,
                                                      uint8_t* Output, size_t N,
                                                      float Scale, int8_t ZeroPoint);

typedef void(MLASCALL MLAS_QUANTIZE_LINEAR_S4_KERNEL)(const float* Input,
                                                      uint8_t* Output, size_t N,
                                                      float Scale, int8_t ZeroPoint);

template <typename InputType, typename FilterType>
struct MLAS_QUANT_KERNEL {
  typedef void(MLASCALL DepthwiseKernel)(const InputType* const* Input,
                                         InputType InputZeroPoint,
                                         const FilterType* Filter,
                                         FilterType FilterZeroPoint, int32_t* Output,
                                         size_t Channels, size_t OutputCount,
                                         size_t KernelSize);
};
...
void MlasSgemmOperation(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB, size_t M,
...
//
// Quantized 8-bit integer/quantized 4-bit integer matrix/matrix multiply dispatch
// structure.
//
...
template <typename InputType, typename FilterType>
void MLASCALL MlasConvDepthwiseKernel(const InputType* const* Input,
                                      InputType InputZeroPoint,
                                      const FilterType* Filter,
                                      FilterType FilterZeroPoint, int32_t* Output,
                                      size_t Channels, size_t OutputCount,
...
template <typename InputType, typename FilterType>
void MLASCALL MlasConvDepthwiseKernelAvx2(const InputType* const* Input,
                                          InputType InputZeroPoint,
                                          const FilterType* Filter,
                                          FilterType FilterZeroPoint,
                                          int32_t* Output, size_t Channels,
                                          size_t OutputCount, size_t KernelSize);
...
inline MLAS_PLATFORM& GetMlasPlatform() {
...
typedef void(MLAS_THREADED_ROUTINE)(void* Context, ptrdiff_t Index);
...
constexpr size_t MlasDivRoundup(size_t up, size_t down) {
 ...
 * @param Work [IN]                Logic for computing a range of iterations [begin,
 * end)
 */
void MlasTrySimpleParallel(MLAS_THREADPOOL* ThreadPool,
...
inline ptrdiff_t MlasGetMaximumThreadCount(MLAS_THREADPOOL* ThreadPool) {
#if defined(BUILD_MLAS_NO_ONNXRUNTIME)
...
inline void MlasPartitionWork(ptrdiff_t ThreadId, ptrdiff_t ThreadCount,
                              size_t TotalWork, size_t* WorkIndex,
...
#pragma warning(push)
// VC++ suggests we can attempt to make 'MlasBitsOfFp32' constexpr, but it is not
// valid.
#pragma warning(disable : 26497)
#endif

MLAS_FORCEINLINE
uint32_t MlasBitsOfFp32(float FloatValue) {
  union {
...
void MLASCALL MlasConvDepthwiseFloat_CHW(const MLAS_CONV_PARAMETERS* Parameters,
                                         const float* Input, const float* Filter,
                                         float* Output, const float* Zeros);
...
#else
  return MLAS_INT32X4{int32_t(Vector[0]), int32_t(Vector[1]), int32_t(Vector[2]),
                      int32_t(Vector[3])};
#endif
}
...
#else
  return MLAS_FLOAT32X4{float(Vector[0]), float(Vector[1]), float(Vector[2]),
                        float(Vector[3])};
#endif
}
...
MLAS_FORCEINLINE
void MlasStoreInt32x4(int32_t* Buffer, MLAS_INT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
MLAS_FORCEINLINE
MLAS_INT32X4
MlasBlendInt32x4(MLAS_INT32X4 Vector1, MLAS_INT32X4 Vector2,
                 MLAS_INT32X4 Selection) {
  return MlasOrInt32x4(MlasAndInt32x4(Vector2, Selection),
                       MlasAndNotInt32x4(Selection, Vector1));
}

template <unsigned ShiftCount>
MLAS_FORCEINLINE MLAS_INT32X4 MlasShiftLeftInt32x4(MLAS_INT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
MLAS_FORCEINLINE
void MlasStoreFloat32x4(float* Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
MLAS_FORCEINLINE
void MlasStoreAlignedFloat32x4(float* Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
  vst1q_f32(Buffer, Vector);
#elif defined(MLAS_SSE2_INTRINSICS)
...
MLAS_FORCEINLINE void MlasStoreLaneFloat32x4(float* Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
  ...
  _mm_store_ss(Buffer,
               _mm_shuffle_ps(Vector, Vector, _MM_SHUFFLE(Lane, Lane, Lane, Lane)));
#elif defined(MLAS_WASM_SIMD_INTRINSICS)
...
MLAS_FORCEINLINE
void MlasStoreLowHalfFloat32x4(float* Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
template <unsigned Lane>
MLAS_FORCEINLINE float MlasExtractLaneFloat32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
template <>
MLAS_FORCEINLINE void MlasStoreLaneFloat32x4<0>(float* Buffer,
                                                MLAS_FLOAT32X4 Vector) {
  _mm_store_ss(Buffer, Vector);
}

template <>
MLAS_FORCEINLINE float MlasExtractLaneFloat32x4<0>(MLAS_FLOAT32X4 Vector) {
  return _mm_cvtss_f32(Vector);
}

template <unsigned Index0, unsigned Index1, unsigned Index2, unsigned Index3>
MLAS_FORCEINLINE MLAS_FLOAT32X4 MlasShuffleFloat32x4(MLAS_FLOAT32X4 Vector) {
...
template <unsigned Index0, unsigned Index1, unsigned Index2, unsigned Index3>
MLAS_FORCEINLINE MLAS_FLOAT32X4 MlasShuffleFloat32x4(MLAS_FLOAT32X4 Vector1,
                                                     MLAS_FLOAT32X4 Vector2) {
  ...
  return __builtin_shuffle(Vector1, Vector2,
                           GEN_INT32X4{Index0, Index1, Index2, Index3});
#else
  return __builtin_shuffle(Vector1, Vector2,
                           MLAS_INT32X4{Index0, Index1, Index2, Index3});
#endif
}
...
#elif defined(MLAS_LSX_INTRINSICS)
  return (MLAS_FLOAT32X4)__lsx_vilvl_w(MlasReinterpretAsInt32x4(Vector2),
                                       MlasReinterpretAsInt32x4(Vector1));
#else
...
#elif defined(MLAS_LSX_INTRINSICS)
  return (MLAS_FLOAT32X4)__lsx_vilvh_w(MlasReinterpretAsInt32x4(Vector2),
                                       MlasReinterpretAsInt32x4(Vector1));
#else
...
MLAS_FORCEINLINE
MLAS_FLOAT32X4
MlasMultiplyAddFloat32x4(MLAS_FLOAT32X4 Vector1, MLAS_FLOAT32X4 Vector2,
                         MLAS_FLOAT32X4 Vector3) {
#if defined(MLAS_NEON_INTRINSICS)
...
MLAS_FORCEINLINE
MLAS_FLOAT32X4
MlasMultiplyAddFloat32x4(MLAS_FLOAT32X4 Vector1, float Scalar2,
                         MLAS_FLOAT32X4 Vector3) {
...
#elif defined(MLAS_NEON32_INTRINSICS)
  Vector1 = vsetq_lane_f32(vgetq_lane_f32(Vector1, 0) / vgetq_lane_f32(Vector2, 0),
                           Vector1, 0);
  Vector1 = vsetq_lane_f32(vgetq_lane_f32(Vector1, 1) / vgetq_lane_f32(Vector2, 1),
                           Vector1, 1);
  Vector1 = vsetq_lane_f32(vgetq_lane_f32(Vector1, 2) / vgetq_lane_f32(Vector2, 2),
                           Vector1, 2);
  Vector1 = vsetq_lane_f32(vgetq_lane_f32(Vector1, 3) / vgetq_lane_f32(Vector2, 3),
                           Vector1, 3);
  return Vector1;
#elif defined(MLAS_SSE2_INTRINSICS)
...
MlasAndFloat32x4(MLAS_FLOAT32X4 Vector1, MLAS_FLOAT32X4 Vector2) {
  ...
  return wasm_v128_and(Vector1, Vector2);
#elif defined(MLAS_LSX_INTRINSICS)
  return MlasReinterpretAsFloat32x4(MlasAndInt32x4(
      MlasReinterpretAsInt32x4(Vector1), MlasReinterpretAsInt32x4(Vector2)));
#else
  return MlasReinterpretAsFloat32x4(MlasAndInt32x4(
      MlasReinterpretAsInt32x4(Vector1), MlasReinterpretAsInt32x4(Vector2)));
#endif
...
MlasOrFloat32x4(MLAS_FLOAT32X4 Vector1, MLAS_FLOAT32X4 Vector2) {
  ...
  return wasm_v128_or(Vector1, Vector2);
#elif defined(MLAS_LSX_INTRINSICS)
  return MlasReinterpretAsFloat32x4(MlasOrInt32x4(
      MlasReinterpretAsInt32x4(Vector1), MlasReinterpretAsInt32x4(Vector2)));
#else
  return MlasReinterpretAsFloat32x4(MlasOrInt32x4(
      MlasReinterpretAsInt32x4(Vector1), MlasReinterpretAsInt32x4(Vector2)));
...
MlasAndNotFloat32x4(MLAS_FLOAT32X4 VectorNot, MLAS_FLOAT32X4 Vector) {
  ...
  return wasm_v128_andnot(Vector, VectorNot);
#elif defined(MLAS_LSX_INTRINSICS)
  return MlasReinterpretAsFloat32x4(MlasAndNotInt32x4(
      MlasReinterpretAsInt32x4(VectorNot), MlasReinterpretAsInt32x4(Vector)));
#else
  return MlasReinterpretAsFloat32x4(MlasAndNotInt32x4(
      MlasReinterpretAsInt32x4(VectorNot), MlasReinterpretAsInt32x4(Vector)));
#endif
...
MLAS_FORCEINLINE
MLAS_FLOAT32X4
MlasBlendFloat32x4(MLAS_FLOAT32X4 Vector1, MLAS_FLOAT32X4 Vector2,
                   MLAS_FLOAT32X4 Selection) {
  return MlasOrFloat32x4(MlasAndFloat32x4(Vector2, Selection),
                         MlasAndNotFloat32x4(Selection, Vector1));
}
...
MLAS_FORCEINLINE
float MlasReduceAddFloat32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON64_INTRINSICS)
...
#elif defined(MLAS_VSX_INTRINSICS)
  Vector = MlasAddFloat32x4(
      Vector, MLAS_FLOAT32X4(vec_splat((__vector long long)Vector, 1)));
...
MLAS_FORCEINLINE
float MlasReduceMinimumFloat32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON64_INTRINSICS)
...
MlasPowerOf2Float32x4(MLAS_FLOAT32X4 Vector) {
  MLAS_INT32X4 emm0 =
      MlasAddInt32x4(MlasCastToInt32x4(Vector), MlasBroadcastInt32x4(127));
...
template <unsigned Lane>
MLAS_FORCEINLINE double MlasExtractLaneFloat64x2(MLAS_FLOAT64X2 Vector) {
  return Vector[Lane];
}
MLAS_FORCEINLINE
MLAS_FLOAT64X2
MlasMultiplyAddFloat64x2(MLAS_FLOAT64X2 Vector1, MLAS_FLOAT64X2 Vector2,
                         MLAS_FLOAT64X2 Vector3) {
  return vec_madd(Vector1, Vector2, Vector3);
}
...
MLAS_FORCEINLINE
void MlasStoreFloat64x2(double* Buffer, MLAS_FLOAT64X2 Vector) {
#if defined(MLAS_SSE2_INTRINSICS)
...
MLAS_FORCEINLINE
void MlasStoreAlignedFloat64x2(double* Buffer, MLAS_FLOAT64X2 Vector) {
#if defined(MLAS_SSE2_INTRINSICS)
...
MLAS_FORCEINLINE
uint64_t MlasReadTimeStampCounter(void) {
#ifdef _WIN32
#if defined(MLAS_TARGET_AMD64_IX86)
...
#ifdef _MSC_VER
extern thread_local std::unique_ptr<uint8_t, decltype(&_aligned_free)>
    ThreadedBufHolder;
#else
...
MLAS_FORCEINLINE void MlasSetInt4Element(uint8_t* Output, size_t ElemIndex,
                                         UnpackedType Value) {
  static_assert(std::is_same_v<UnpackedType, uint8_t> ||
                std::is_same_v<UnpackedType, int8_t>);
...
template <typename UnpackedType>
MLAS_FORCEINLINE void MlasPackInt4Elements(uint8_t* Output, UnpackedType ValueLow,
                                           UnpackedType ValueHigh) {
  static_assert(std::is_same_v<UnpackedType, uint8_t> ||
                std::is_same_v<UnpackedType, int8_t>);
