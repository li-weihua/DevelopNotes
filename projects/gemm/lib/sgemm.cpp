...
void MlasSgemmMultiplyBeta(float* C, size_t CountM, size_t CountN, size_t ldc,
                           float beta)
/*++
...
--*/
{
  ...
  while (CountM-- > 0) {
    ...
    while (n >= 4) {
      MlasStoreFloat32x4(c,
                         MlasMultiplyFloat32x4(MlasLoadFloat32x4(c), BetaBroadcast));
      c += 4;
      n -= 4;
    }
...
void MlasSgemmTransposeA(float* D, const float* A, size_t lda, size_t CountY,
                         size_t CountX)
/*++
...
void MlasSgemmCopyPackB(float* D, const float* B, size_t ldb, size_t CountX,
                        size_t CountY)
/*++
...
#if defined(MLAS_NEON_INTRINSICS)
    float32x4x4_t ZeroFloat32x4x4 = {ZeroFloat32x4, ZeroFloat32x4, ZeroFloat32x4,
                                     ZeroFloat32x4};
#endif
...
template <unsigned N>
inline void MlasSgemmTransposePackBNx4(float* D, const float* B, size_t ldb)
/*++
...
void MlasSgemmTransposePackB(float* D, const float* B, size_t ldb, size_t CountY,
                             size_t CountX)
/*++
...
MLAS_FORCEINLINE
float* MlasSgemmKernelLoop(const float* A, const float* B, float* C, size_t CountK,
                           size_t CountM, size_t CountN, size_t lda, size_t ldc,
                           float alpha, bool ZeroMode)
/*++
...
--*/
{
  while (CountM > 0) {
    ...
    RowsHandled = GetMlasPlatform().GemmFloatKernel(A, B, C, CountK, CountM, CountN,
                                                    lda, ldc, alpha, ZeroMode);
#else
    if (ZeroMode) {
      RowsHandled =
          MlasSgemmKernelZero(A, B, C, CountK, CountM, CountN, lda, ldc, alpha);
    } else {
      RowsHandled =
          MlasSgemmKernelAdd(A, B, C, CountK, CountM, CountN, lda, ldc, alpha);
    }
#endif
...
--*/
{
  ...
  MLAS_DECLSPEC_ALIGN(float PanelB[MLAS_SGEMM_STRIDEN * MLAS_SGEMM_STRIDEK],
                      16 * sizeof(float));
  ...
  //
  // Handle the case when both B and C are column-vectors that are contiguous in
  // memory. Because transposition of such vectors doesn't change their layout, and
  // Transpose(A*B) = Transpose(B) * Transpose(A), we can apply the same 'small-M'
  // optimization as above, with A and B flipped.
  //

  if (N == 1 && ldb == 1 && ldc == 1 && alpha == 1.0f &&
      (beta == 0.0f || beta == 1.0f)) {
#if defined(MLAS_TARGET_AMD64)
  ...
  for (size_t n = 0; n < N; n += CountN) {
    ...
    for (size_t k = 0; k < K; k += CountK) {
      ...
      if (TransA == CblasNoTrans) {
        MlasSgemmKernelLoop(A + k, PanelB, c, CountK, M, CountN, lda, ldc, alpha,
                            ZeroMode);
        ...
        while (RowsRemaining > 0) {
          ...
          size_t RowsTransposed =
              std::min(RowsRemaining, size_t(MLAS_SGEMM_TRANSA_ROWS));
          ...
          c = MlasSgemmKernelLoop(PanelA, PanelB, c, CountK, RowsTransposed, CountN,
                                  CountK, ldc, alpha, ZeroMode);
        }
      }
...
void MlasSgemmPackedOperation(CBLAS_TRANSPOSE TransA, size_t M, size_t RangeStartN,
                              size_t RangeCountN, size_t K, float alpha,
                              const float* A, size_t lda, const void* PackedB,
                              size_t AlignedN, float beta, float* C, size_t ldc)
/*++
...
--*/
{
  ...
  for (size_t n = 0; n < RangeCountN; n += CountN) {
    ...
    for (size_t k = 0; k < K; k += CountK) {
      ...
      if (TransA == CblasNoTrans) {
        MlasSgemmKernelLoop(A + k, pb, c, CountK, M, CountN, lda, ldc, alpha,
                            ZeroMode);
        ...
        while (RowsRemaining > 0) {
          ...
          c = MlasSgemmKernelLoop(PanelA, pb, c, CountK, RowsTransposed, CountN,
                                  CountK, ldc, alpha, ZeroMode);
        }
      }
...
Return Value:
    ...
    None.
--*/
void MlasSgemmThreaded(const ptrdiff_t ThreadCountM, const ptrdiff_t ThreadCountN,
  ...
  if (DataParams->BIsPacked) {
    MlasSgemmPackedOperation(TransA, RangeCountM, RangeStartN, RangeCountN, K,
                             DataParams->alpha, A, lda, DataParams->B,
                             BlockedN * MLAS_SGEMM_STRIDEN_THREAD_ALIGN,
                             DataParams->beta, C, ldc);

  } else {
    const size_t ldb = DataParams->ldb;

    const float* B = (const float*)DataParams->B +
                     RangeStartN * ((TransB == CblasNoTrans) ? 1 : ldb);
...
#endif
void MLASCALL MlasGemmBatch(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB, size_t M,
                            size_t N, size_t K, const MLAS_SGEMM_DATA_PARAMS* Data,
                            size_t BatchSize, MLAS_THREADPOOL* ThreadPool) {
  //
  ...
  if (Complexity <
      double(MLAS_SGEMM_THREAD_COMPLEXITY * GetMlasPlatform().MaximumThreadCount)) {
    TargetThreadCount =
        ptrdiff_t(Complexity / double(MLAS_SGEMM_THREAD_COMPLEXITY)) + 1;
  } else {
  ...
  for (ptrdiff_t tid = 0; tid < ThreadsPerGemm * static_cast<ptrdiff_t>(BatchSize);
       tid++) {
    ptrdiff_t GemmIdx = tid / ThreadsPerGemm;
    ptrdiff_t ThreadIdx = tid % ThreadsPerGemm;
    MlasSgemmThreaded(ThreadCountM, ThreadCountN, TransA, TransB, M, N, K,
...
size_t MLASCALL MlasGemmPackBSize(size_t N, size_t K)
/*++
...
--*/
{
  ...
  const size_t AlignedN = (N + MLAS_SGEMM_STRIDEN_THREAD_ALIGN - 1) &
...
void MLASCALL MlasGemmPackB(CBLAS_TRANSPOSE TransB, size_t N, size_t K,
                            const float* B, size_t ldb, void* PackedB)
/*++
